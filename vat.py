...
def VAT(input_op, network, network_to_approximate=None, xi=1e-6, num_approximation=1, clip_value_min=1e-30, dtype=tf.float32):
    """
    ===input===
    input_op               : input operation of network
    network                : function that receives input_op and returns the logits (i.e., the output without softmax.)
    ...
    num_approximation      : number of iteration to approximate the virtual adversarial perturbation.
    ...
    axis_without_batch_size = tuple(range(1,len(input_op.get_shape())))
    ...
    plain_softmax = tf.nn.softmax(network_to_approximate(input_op))
    perturbation = xi * normalized(tf.random_normal(shape=tf.shape(input_op), dtype=dtype))
    for i in range(num_approximation):
        softmax_accommodating_perturbation = tf.nn.softmax(network_to_approximate(input_op+perturbation))
    ...
    current_softmax = tf.nn.softmax(network(input_op)) if not isSameNetwork else plain_softmax
    current_softmax = tf.stop_gradient(current_softmax)
    vat_perturbation = tf.stop_gradient(vat_perturbation)
    vat_softmax = tf.nn.softmax(network(input_op + vat_perturbation))
